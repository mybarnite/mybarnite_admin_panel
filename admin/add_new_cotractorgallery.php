<? include("header.php");
 ...
 if(isset($_REQUEST['add_new_banner']))
 {
	  ...
	  $banner_image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['banner_image']['name']);
       ...
       $succ= mysql_query("insert into tbl_gallery set `banner_image`='".$banner_image."',`heading`='".$banner_heading."',`desc`='".$banner_desc."'") or die(mysql_error());	 
		   ...
		   else
		 {
		   ...
		   ?>
		  <script>window.location.href="add_new_cotractorgallery.php?msg=<?=$_SESSION['msg'];?>&t=true"</script>
		   <?
