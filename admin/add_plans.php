				...
				<div class="box span12">
                 <script src="js/functionsforcheckall.js"></script>
					<div class="box-header well" data-original-title>
						<h2><i class="icon-list"></i>Add Plans</h2>
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  <thead>
							  <tr>
								...
								<th>Philosophy:</th>
                                <th>Message Philosophy</th>
								<th>Cash Links:</th>
                                <th>Message Cash Links</th>
                                <th>Sharing:</th>
                                <th>Message Sharing</th>
                                <th>Earn Traffic Exchange Credits:</th>
                                <th>Message Earn Traffic Exchange Credits</th>
                                <th>Services:</th>
                                <th>Message Services</th>
                               
								  
							  </tr>
						  </thead>   
						  <tbody>
                          <? $sel_banner_query= mysql_query("select * from add_plans");
                           ...
                           <?php /*?> <td><input type="checkbox" name="chk[]" value="<?=$fetch_banner['id']; ?>" /></td><?php */?>
                            <td> <a class="btn btn-info" href="update_add_plans.php?banner_id=<?=$fetch_banner['id']; ?>&t=true">
							<i class="icon-pencil" title="Edit"></i></a>
								<td class="center"><?=$fetch_banner['philosophy'];?></td>
								<td class="center"><?=$fetch_banner['philosophy_message'];?></td>
                                <td class="center"><?=$fetch_banner['cash_link'];?></td>
								<td class="center"><?=$fetch_banner['cash_link_message'];?></td>
                                <td class="center"><?=$fetch_banner['sharing'];?></td>
								<td class="center"><?=$fetch_banner['sharing_message'];?></td>
								<td class="center"><?=$fetch_banner['earn_traffic_exchange'];?></td>
								<td class="center"><?=$fetch_banner['earn_traffic_exchange_message'];?></td>
								<td class="center"><?=$fetch_banner['services'];?></td>
								<td class="center"><?=$fetch_banner['services_message'];?></td>
