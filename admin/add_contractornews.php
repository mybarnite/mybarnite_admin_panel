<? include("header.php");
 ...
 {
	 echo $page_heading=trim($_REQUEST['page_heading']);
	 echo $page_content=trim($_REQUEST['page_content']);
      $image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['image']['name']);
       move_uploaded_file($_FILES["image"]["tmp_name"],"product_images/".$image);
     
   
	  
       $succ= mysql_query("insert into tbl_contractornews set news_heading='".$page_heading."',news_description='".$page_content."'");	 
