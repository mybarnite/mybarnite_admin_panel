...
include("header.php");
	...
	if(isset($_REQUEST['delete']))
	{
		...
		?>
		   <script>window.location.href="event_list.php?msg=<?=$_SESSION['msg'];?>&t=true";</script>
		<?php
	}
						...
						<div class="box span12">
							<script src="js/functionsforcheckall.js"></script>
								<div class="box-header well" data-original-title>
										<h2><i class="icon-list"></i> Manage Business Events</h2>
										<!--<li><a class="ajax-link" href="welcome.php"><span class="hidden-tablet" style=" color:#F00;">Back To Barnite</span></a></li>-->
											...
											<table class="table table-striped table-bordered bootstrap-datatable datatable">
												<thead>
													<tr>
														...
														<th colspan="3">
															<label style="float: left;">Change Event status :</label>
															<select name="eventtype" id="eventtype" onchange="changeEventType();" style="width: 100px;margin-left: 10px;float: left;">
													...
													<tr>
														<th width="5%"><input type="checkbox" id="selecctall" onClick="selectAll(this)" style="margin:0 4px 0;"/>	All</li></th>
														<th>Event Name</th>
														<th>Bar Name</th>
														<th>Start Date</th>
														<th>End Date</th>
													...
													<tr>
														...
														<th><input type="text" id="EventName" class="form-group" style="width: 100px;"/></th>
														<th><input type="text" id="BarName" class="form-group" style="width: 100px;"/></th>
														<th>
															<input type="date" name="startDate" id="startDate" class="form-group startDate" style="width:123px;"/>
														</th>
														<th>
															<input type="date" name="endDate" id="endDate" class="form-group endDate" style="width:122px;"/>
														</th>
														<th></th>
														<th></th>
														<th>
															<select name="searchEventType" class="searchEventType" id="searchEventType" style="width:100px;">
																<option value="0">Select</option>
																<option value="1">Booking Available</option>
																<option value="2">Fully Booked</option>
															</select>
														</th>
...
function changeEventType()
{
	...
	$.ajax(
      {
        url: "ajax/changeEventType.php", 
        type:"POST",
        data:{eventType:eventType,eventids:myCheckboxes},
        success: function(response){
			window.location = "event_list.php"
    ...
    $.ajax(
      {
        ...
        type:"POST",
        data:{
          pageNo:$("#userPage").val(),EventName:$('#EventName').val(),BarName:$('#BarName').val(),startDate:$( "#startDate" ).val(),endDate:$("#endDate").val(),searchEventType:$("#searchEventType option:selected").val()}
        ,
        success: function(response){
          //alert(response);
