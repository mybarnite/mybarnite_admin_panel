				...
				<div class="box span12">
                 <script src="js/functionsforcheckall.js"></script>
					<div class="box-header well" data-original-title>
						<h2><i class="icon-list"></i>UPCOMING EVENTS </h2>
                    ...
                    <form action="" method="post" onSubmit="return validate()">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
                          <?php $sel_banner_query= mysql_query(" SELECT * FROM tbl_events WHERE event_start >= CURDATE() and end_time >= CURTIME() ");
							...
							<td></td>
                            <td> <a class="btn btn-info" href="update_upcoming_event.php?banner_id=<?=$fetch_banner['id']; ?>&t=true">
							<i class="icon-pencil" title="Edit"></i></a>
								<td class="center"><?=$fetch_banner['event_name'];?></td>
								<td class="center"><?=$fetch_banner['event_description'];?></td>
								<td class="center">From : <?=$fetch_banner['event_start'];?> <?=$fetch_banner['start_time'];?> To : <?=$fetch_banner['event_end'];?> <?=$fetch_banner['end_time'];?></td>
