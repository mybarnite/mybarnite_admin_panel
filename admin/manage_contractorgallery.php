<? include("header.php");
  ...
  foreach($chk as $id)
  {
	  $tmp_img_qury= mysql_query("select * from tbl_gallery where id='".$id."'");
	$fetch_img_name=mysql_fetch_array($tmp_img_qury);
	$file_to_delete="product_images/".$fetch_img_name['banner_image'];
	unlink($file_to_delete);
  $succ_del=mysql_query("delete from tbl_gallery where id='".$id."'");
  }
   if($succ_del)
  {
  ...
  ?>
	   <script>window.location.href="manage_contractorgallery.php?msg=<?=$_SESSION['msg'];?>&t=true";</script>
	  <?
  }
  ...
  foreach($chk as $id)
  {
  mysql_query("update tbl_gallery set status='Active' where id='".$id."'");
  ...
  foreach($chk as $id)
  {
  mysql_query("update tbl_gallery set status='Inactive' where id='".$id."'");
				...
				<div class="box span12">
                 <script src="js/functionsforcheckall.js"></script>
					<div class="box-header well" data-original-title>
						<h2><i class="icon-list"></i> Gallery Management</h2>
                         <?php if($_REQUEST['t']) { ?><li><a class="ajax-link" href="welcome.php"><span class="hidden-tablet" style=" color:#F00;">Back To Skilled Freelance Work</span></a></li><?php
						}
						?>
						<div align="right">
							<a href="add_new_cotractorgallery.php?t=true" class="btn btn-setting btn-round">Add New Image</a>
                    ...
                    <form action="" method="post" onSubmit="return validate()">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
                          <? $sel_banner_query= mysql_query("select * from tbl_gallery");
                            ...
                            <td> <a class="btn btn-info" href="update_contractorgallery.php?banner_id=<?=$fetch_banner['id']; ?>&t=true">
							<i class="icon-pencil" title="Edit"></i></a></td>
                         		<td class="center"><img src="../images/slider/<?=$fetch_banner['banner_image'];?>" width="100" height="100" /></td>
								<td class="center"><?=$fetch_banner['heading'];?></td>
								<td class="center"><?=$fetch_banner['desc'];?></td>
                             		<td class="center">
