<? include("header.php");
  ...
  foreach($chk as $id)
  {
	  $tmp_img_qury= mysql_query("select * from tbl_main_category where id='".$id."'");
	$fetch_img_name=mysql_fetch_array($tmp_img_qury);
	$file_to_delete="product_images/".$fetch_img_name['category_image'];
	unlink($file_to_delete);
  $succ_del=mysql_query("delete from tbl_main_category where id='".$id."'");
  }
   if($succ_del)
  {
  ...
  ?>
	   <script>window.location.href="manage_main_category.php?msg=<?=$_SESSION['msg'];?>";</script>
	  <?
  }
  ...
  foreach($chk as $id)
  {
  mysql_query("update tbl_main_category set status='Active' where id='".$id."'");
  ...
  foreach($chk as $id)
  {
  mysql_query("update tbl_main_category set status='Inactive' where id='".$id."'");
                    ...
                    <form action="" method="post" onSubmit="return validate()">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
                         ...
                         <!--<th width="167">Category Image</th>-->
						  ...
						  <tbody>
                          <? $sel_deptmet_query= mysql_query("select * from tbl_main_category");
