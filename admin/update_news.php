...
$page_query=mysql_query("select * from tbl_freelance_tab where id='".$_REQUEST['page_id']."'");
	 ...
	 if($_FILES['image']['name'])
	 {
      $image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['image']['name']);
       move_uploaded_file($_FILES["image"]["tmp_name"],"../images/icons/".$image);
	    $succ= mysql_query("update tbl_freelance_tab set image='".$image."' where id='".$_REQUEST['page_id']."'");	 
	 
	 }
	  if($image == ''){
		$succ= mysql_query("update tbl_freelance_tab set news_heading='".$page_heading."',news_description='".$page_content."' where id='".$_REQUEST['page_id']."'");	
	  }
	  else{
	  
		$succ= mysql_query("update tbl_freelance_tab set news_heading='".$page_heading."',news_description='".$page_content."', image='".$image."' where id='".$_REQUEST['page_id']."'");	
	  }
		   ...
		   else
		 {
		   ...
		   ?>
		  <script>window.location.href="update_news.php?page_id=1 && msg=<?=$_SESSION['msg'];?>"</script>
		   <?
	 }
