...
$page_query=mysql_query("select * from tbl_events where id='".$_REQUEST['page_id']."'");
	 ...
	 if($_FILES['image']['name'])
	 {
      $image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['image']['name']);
       move_uploaded_file($_FILES["image"]["tmp_name"],"../images/".$image);
	    $succ= mysql_query("update tbl_events set image='".$image."' where id='".$_REQUEST['page_id']."'");	 
	 
	 }
	  if($image == ''){
		$succ= mysql_query("update tbl_events set news_heading='".$page_heading."',news_description='".$page_content."' where id='".$_REQUEST['page_id']."'");	
	  }
	  else{
	  
		$succ= mysql_query("update tbl_events set news_heading='".$page_heading."',news_description='".$page_content."', image='".$image."' where id='".$_REQUEST['page_id']."'");	
	  }
