...
include("functions/functions.php");
	 ...
	 if(isset($_REQUEST['add_new_p_type']))
 {
	...
	$c_type=addslashes(trim($_REQUEST['c_type']));
	...
	/*	$data = array(
		...
		*/
		...
		$succ = mysql_query("INSERT INTO `work_cat_list` VALUES(NULL,'".$p_type."','".$c_type."')")or die(mysql_error());
	 
	    if($succ)
		{
		   $_SESSION['msg']="New Work Added to Category ".$p_type;	
		}
		else
		{
				  ...
				  <div class="box-content">
                    ...
                    <tr><td align="center" style="color:#F00;"><strong><?php echo $_REQUEST['msg'];?></strong></td></tr>
                     <tr><td align="center" style="color:#F00;">&nbsp;</td></tr>
						...
						<tr>
							<td><label class="control-label" for="typeahead"> Work Title </label></td>
							<td><input type="text"  class="span6 typeahead"   name="p_type"  id="page_heading"  size="40" /></td>
                        </tr>
                        
                        <tr>
							<td><label class="control-label" for="typeahead"> Select Category to add </label></td>
							<td>
                            <select name="c_type">
                            	<?php 
							...
							if( $num_rows > 0){
						     while($fetch_content=mysql_fetch_array($sel_page_query))
							 {
								...
								$category = $fetch_content['category'];
	
								?>
                              <option value="<?php echo $cont_Id;?>"> <?php echo $category;?></option>
                              <?php 
							  }
							}
							  ?>  
