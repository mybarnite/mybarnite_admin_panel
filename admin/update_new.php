...
$page_query=mysql_query("select * from tbl_news where id='".$_REQUEST['page_id']."'");
	 ...
	 if($_FILES['image']['name'])
	 {
      $image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['image']['name']);
       move_uploaded_file($_FILES["image"]["tmp_name"],"../images/icons/".$image);
	    $succ= mysql_query("update tbl_news set image='".$image."' where id='".$_REQUEST['page_id']."'");	 
	 
	 }
	  if($image == ''){
		$succ= mysql_query("update tbl_news set news_heading='".$page_heading."',news_description='".$page_content."' where id='".$_REQUEST['page_id']."'");	
	  }
	  else{
	  
		$succ= mysql_query("update tbl_news set news_heading='".$page_heading."',news_description='".$page_content."', image='".$image."' where id='".$_REQUEST['page_id']."'");	
	  }
		   ...
		   else
		 {
		   ...
		   ?>
		  <script>window.location.href="manage_news.php?msg=<?=$_SESSION['msg'];?>"</script>
		   <?
	 }
                        ...
                        <td>
							<?php 
								if ($fetch_content['image'] == ''){}
									...
									//echo '<img src="../images/icons/'.$fetch_content['image'].'"  height="80px;">';
								}

							?>
							<input type="hidden" class="span6 typeahead"  name="image" id="image"  size="28" /></td>
                         </tr>
                        <tr >
                        <td ><label class="control-label" for="typeahead">Description</label></td>
                        <td>
                        <textarea name="page_content" width="200px" rows="10" cols="30"><?=$fetch_content['news_description'];?></textarea>
