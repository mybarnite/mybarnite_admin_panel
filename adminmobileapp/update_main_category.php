<? include("header.php");
  ...
  $sel_cat=mysql_query("select * from tbl_elearning_cpd where id='".$_REQUEST['cat_id']."'");
...

 {
	 ...
	 $meta_descr=trim($_REQUEST['meta_descr']);
       ...
       move_uploaded_file($_FILES["category_image"]["tmp_name"],"product_images/".$category_image);
	     ...
	     mysql_query("update tbl_elearning_cpd set category_image='".$category_image."' where id='".$_REQUEST['cat_id']."'");

	   }

       $succ= mysql_query("update tbl_elearning_cpd set category_name='".$category_name."',category_descr='".$category_descr."',meta_title='".$meta_title."',meta_keywords='".$meta_keywords."',meta_descr='".$meta_descr."' where id='".$_REQUEST['cat_id']."'");	 
