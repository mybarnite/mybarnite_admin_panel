...
$page_query=mysql_query("select * from tbl_contractor_staticpage  where id='".$_REQUEST['page_id']."'");
$fetch_content=mysql_fetch_array($page_query);
 if(isset($_REQUEST['add_new_page']))
 {
	 ...
	 $page_content=trim($_REQUEST['page_content']);
		...
		{
		 $image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['pageimage']['name']);
         move_uploaded_file($_FILES["pageimage"]["tmp_name"],"product_images/".$image);
		 
	   	 $succ= mysql_query("update tbl_contractor_staticpage  set image='".$image."' where id='".$_REQUEST['page_id']."'");
		...
		if($_FILES['banner_image']['name']!='')
		{
		 ...
		 $succ= mysql_query("update tbl_contractor_staticpage  set banner_image='".$banner_image."' where id='".$_REQUEST['page_id']."'");
	 
      }
		
       $succ= mysql_query("update tbl_contractor_staticpage  set page_content='".$page_content."' where id='".$_REQUEST['page_id']."'");	 
	 
	       if($succ)
		 {
		   
		   $_SESSION['msg']="Page is Successfully Updated";	
		   }
		   else
		 {
		   $_SESSION['msg']="Page is Not Updated";	
		   }
		   ?>
			...
			<div id="content" class="span10">
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-edit"></i>Edit Static Page</h2>
						<div align="right">
                    ...
                    <tr><td align="center" style="color:#F00;"><strong><? echo $_REQUEST['msg'];?></strong></td></tr>
                     <tr><td align="center" style="color:#F00;">&nbsp;</td></tr>
                        ...
                        <td><strong><?=$fetch_content['page_title'];?></strong></td>
                         </tr>
                        ...
                        <td><input type="file" class="span6 typeahead"  name="pageimage" id="image"  size="40" /></td>
                         </tr>
                         
                             <tr >
                        <td ><label class="control-label" for="typeahead">Banner Image</label></td>
                        <td><input type="file" class="span6 typeahead"  name="banner_image" id="image"  size="40" /></td>
                         </tr>
                         
                           <tr>
                        <td ><label class="control-label" for="typeahead">&nbsp;</label></td>
                        <td>
                        <input type="submit" name="add_new_page" value="Edit Static Page" class="btn btn-primary" onclick="" />
