<? include("header.php");
 ...
 if(isset($_REQUEST['add_new_category']))
 {
	 ...
	 $category_descr=trim($_REQUEST['video']);
   ...
   $query= mysql_query("select * from tbl_subcategory where subcategory_name='".$subcategory_name."' and subcategory_id='".$_REQUEST['sub_cat']."'");
     ...
     $_SESSION['msg']="SubCategory is Already Exist In this Category";	
	   ?>
		  <script>window.location.href="add_new_clasified.php?cat_id=<?=$_REQUEST['cat_id'];?>&sub_cat=<?=$_REQUEST['sub_cat'];?>&msg=<?=$_SESSION['msg'];?>"</script>
		   <?
	 }
	 else
	 {
	  ...
	  $subcat_image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['subcat_image']['name']);
       ...
       $succ= mysql_query("insert into tbl_clasified set subcat_name='".$subcategory_name."',subcat_image='".$subcat_image."',url='".$url."', video='".$video."',category_descr='".$category_descr."',category_id='".$_REQUEST['cat_id']."',meta_title='".$meta_title."',meta_keywords='".$meta_keywords."',meta_descr='".$meta_descr."'");	 
		   ...
		   else
		 {
		   ...
		   ?>
		  <script>window.location.href="add_new_services.php?cat_id=<?=$_REQUEST['cat_id'];?>&sub_cat=<?=$_REQUEST['sub_cat'];?>&msg=<?=$_SESSION['msg'];?>"</script>
		   <?
	 }
 }
?>
                    ...
                    <tr><td align="center" style="color:#F00;"><strong><? echo $_REQUEST['msg'];?></strong></td></tr>
                     <tr><td align="center" style="color:#F00;">&nbsp;</td></tr>
                        ...
                        <td width="30%">&nbsp;</td>
						 ...
						 <tr >
                        <td ><label class="control-label" for="typeahead">Service Name*</label></td>
