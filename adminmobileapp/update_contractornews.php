...
$page_query=mysql_query("select * from tbl_contractornews where id='".$_REQUEST['page_id']."'");
	 ...
	 if($_FILES['image']['name'])
	 {
      $image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['image']['name']);
       move_uploaded_file($_FILES["image"]["tmp_name"],"product_images/".$image);
	    $succ= mysql_query("update tbl_contractornews set image='".$image."' where id='".$_REQUEST['page_id']."'");	 
	 
	 }
	  
       $succ= mysql_query("update tbl_contractornews set news_heading='".$page_heading."',news_description='".$page_content."' where id='".$_REQUEST['page_id']."'");	 
	 
	       if($succ)
		 {
		   
		   $_SESSION['msg']="News is Successfully Updated";	
		   }
		   else
		 {
		   $_SESSION['msg']="News is Not Updated";	
		   }
		   ?>
			...
			<div id="content" class="span10">
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
					  ...
					  <div align="right">
                       	<a href="manage_contractornews.php?t=true" class="btn btn-setting btn-round">Back To List</a>						</div>
				  </div>
