<? include("header.php");
 ...
 if(isset($_REQUEST['add_new_page']))
 {
   ...
   $query= mysql_query("select * from tbl_static_page where page_title='".$page_title."'");
     ...
     $_SESSION['msg']="This Page is already added";	
	   ?>
		  <script>window.location.href="add_new_static_page.php?msg=<?=$_SESSION['msg'];?>"</script>
		   <?
	 }
	 else
	 {
       ...
       $succ= mysql_query("insert into tbl_static_page set page_title='".$page_title."',page_content='".$page_content."',image='".$image."'");	 
