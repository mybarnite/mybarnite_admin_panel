...
$page_query=mysql_query("select * from tbl_offers where id='".$_REQUEST['page_id']."'");
	 ...
	 if($_FILES['image']['name'])
	 {
      $image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['image']['name']);
       move_uploaded_file($_FILES["image"]["tmp_name"],"../images/".$image);
	    $succ= mysql_query("update tbl_offers set image='".$image."' where id='".$_REQUEST['page_id']."'");	 
	 
	 }
	  if($image == ''){
		$succ= mysql_query("update tbl_offers set news_heading='".$page_heading."',news_description='".$page_content."' where id='".$_REQUEST['page_id']."'");	
	  }
	  else{
	  
		$succ= mysql_query("update tbl_offers set news_heading='".$page_heading."',news_description='".$page_content."', image='".$image."' where id='".$_REQUEST['page_id']."'");	
	  }
		   ...
		   else
		 {
		   ...
		   ?>
		  <script>window.location.href="manage_offers.php?msg=<?=$_SESSION['msg'];?>"</script>
		   <?
	 }
			...
			<div id="content" class="span10">
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
					  ...
					  <div align="right">
                       	<a href="manage_offers.php" class="btn btn-setting btn-round">Back To List</a>						</div>
				  </div>
