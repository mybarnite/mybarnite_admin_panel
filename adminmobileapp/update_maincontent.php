...
$banner_query=mysql_query("select * FROM maincontent where id='".$_REQUEST['banner_id']."'");
$fetch_banner=mysql_fetch_array($banner_query);
 if(isset($_REQUEST['add_new_banner']))
 {
	 ...
	 $succ = mysql_query("update maincontent set  `heading`='".$banner_heading."' , `message`='".$banner_desc."' where id='".$_REQUEST['banner_id']."'")or die(mysql_error());	 
		 ...
		 $_SESSION['msg']="Message is Updated";
		   ...
		   ?>
		  <script>window.location.href="update_maincontent.php?msg=<?=$_SESSION['msg'];?>&t=true"</script>
		   <?php
	
 }
 ?><script>window.location.href="maincontent.php?msg=<?=$_SESSION['msg'];?>&t=true"</script>
<?php }
?>
	<!-- topbar ends -->
    <script>
