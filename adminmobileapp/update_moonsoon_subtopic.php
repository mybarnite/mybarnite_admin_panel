...
$banner_query=mysql_query("select * FROM moonsoon_subtopic where id='".$_REQUEST['banner_id']."'");
$fetch_banner=mysql_fetch_array($banner_query);
 if(isset($_REQUEST['add_new_banner']))
 {
	 ...
	 $succ = mysql_query("update moonsoon_subtopic set  `heading`='".$banner_heading."' , `message`='".$banner_desc."' where id='".$_REQUEST['banner_id']."'")or die(mysql_error());	 
		 ...
		 $_SESSION['msg']="Message is Updated";
		   ...
		   ?>
		  <script>window.location.href="update_moonsoon_subtopic.php?msg=<?=$_SESSION['msg'];?>&t=true"</script>
		   <?php
	
 }
 ?><script>window.location.href="moonsoon_subtopic.php?msg=<?=$_SESSION['msg'];?>&t=true"</script>
<?php }
?>
	<!-- topbar ends -->
    <script>
