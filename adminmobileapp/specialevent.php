<? include("header.php");
  ...
  foreach($chk as $id)
  {
	  $tmp_img_qury= mysql_query("select * from slider_images_tb where id='".$id."'");
	$fetch_img_name=mysql_fetch_array($tmp_img_qury);
	$file_to_delete="images/sliderimages/".$fetch_img_name['banner_image'];
	unlink($file_to_delete);
  $succ_del=mysql_query("delete from banner_content where id='".$id."'");
  }
   if($succ_del)
  {
				...
				<div class="box span12">
                 <script src="js/functionsforcheckall.js"></script>
					<div class="box-header well" data-original-title>
						<h2 style="color:#fff;"><i class="icon-list"></i>Special Event</h2>
                         <?php if($_REQUEST['t']) { ?><li><a class="ajax-link" href="welcome.php"><span class="hidden-tablet" style=" color:#F00;">Back To Bar Night</span></a></li><?php
						}
						?>
						<div align="right">
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  <thead>
							  <tr>
                                   ...
                                   <th style="color:#fff;" width="24">Edit</th>
                                  
                                  <th style="color:#fff;" width="214"> Images</th>
								<th style="color:#fff;">Heading</th>
								<th style="color:#fff;">Description</th>
								<th style="color:#fff;">Heading 2</th>
								<th style="color:#fff;">Description 2</th>
								
							  </tr>
						  </thead>   
						  <tbody>
                          <? $sel_banner_query= mysql_query("select * from special_event");
                            ...
                            <td> <a class="btn btn-info" href="update_specialevent.php?banner_id=<?=$fetch_banner['id']; ?>&t=true">
							<i class="icon-pencil" title="Edit"></i></a></td>
                         		<td class="center"><img src="../adminimages/specialevent/<?php echo $fetch_banner['banner_image'];?>" width="100" height="100" /></td>
								<td class="center"><?=$fetch_banner['heading'];?></td>
								<td class="center"><?=$fetch_banner['desc'];?></td>
								
								<td class="center"><?=$fetch_banner['heading2'];?></td>
								<td class="center"><?=$fetch_banner['desc2'];?></td>
