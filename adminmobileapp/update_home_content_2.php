...
$banner_query=mysql_query("select * FROM home_content_2 where id='".$_REQUEST['banner_id']."'");
$fetch_banner=mysql_fetch_array($banner_query);
 if(isset($_REQUEST['add_new_banner']))
 {
	 ...
	 $succ = mysql_query("update home_content_2 set  `heading`='".$banner_heading."' , `message`='".$banner_desc."' where id='".$_REQUEST['banner_id']."'")or die(mysql_error());	 
		 ...
		 $_SESSION['msg']="Message is Updated";
		   ...
		   ?>
		  <script>window.location.href="update_home_content_2.php?msg=<?=$_SESSION['msg'];?>&t=true"</script>
		   <?php
	
 }
 ?><script>window.location.href="home_content_2.php?msg=<?=$_SESSION['msg'];?>&t=true"</script>
<?php }
?>
	<!-- topbar ends -->
    <script>
