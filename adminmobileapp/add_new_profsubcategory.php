 ...
 if(isset($_REQUEST['add_new_category']))
 {
	 ...
	 $meta_descr=trim($_REQUEST['meta_descr']);
	  $url  =sluggify($_REQUEST['subcategory_name']);
	   $pro = "select subcategory_name from tbl_professionalsubcategory where subcategory_name = '".$subcategory_name."'";
   ...
   $query= mysql_query("select * from tbl_professionalsubcategory where subcategory_name='".$subcategory_name."' and category_id='".$_REQUEST['cat_id']."'");
     ...
     $_SESSION['msg']="SubCategory is Already Exist In this Category";	
	   ?>
		  <script>window.location.href="add_new_profsubcategory.php?cat_id=<?=$_REQUEST['cat_id'];?>&msg=<?=$_SESSION['msg'];?>"</script>
		   <?
	 }
	 else
	 {
	  ...
	  $subcat_image = md5(uniqid(rand(), true)).'.'.file_ext($_FILES['subcat_image']['name']);
       ...
       $succ= mysql_query("insert into tbl_professionalsubcategory set subcategory_name='".$subcategory_name."',subcat_image='".$subcat_image."',url='".$url."',category_id='".$_REQUEST['cat_id']."',meta_title='".$meta_title."',meta_keywords='".$meta_keywords."',meta_descr='".$meta_descr."'");	 
