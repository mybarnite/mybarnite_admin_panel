<? include("header.php");
  ...
  foreach($chk as $id)
  {
	  $tmp_img_qury= mysql_query("select * from tbl_banner where id='".$id."'");
	$fetch_img_name=mysql_fetch_array($tmp_img_qury);
	$file_to_delete="product_images/".$fetch_img_name['banner_image'];
	unlink($file_to_delete);
  $succ_del=mysql_query("delete from tbl_banner where id='".$id."'");
  }
   if($succ_del)
  {
  ...
  foreach($chk as $id)
  {
  mysql_query("update tbl_banner set status='Active' where id='".$id."'");
  ...
  foreach($chk as $id)
  {
  mysql_query("update tbl_banner set status='Inactive' where id='".$id."'");
			...
			<!-- content starts -->
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
						...
						<div align="right">
							<a href="add_new_banner.php" class="btn btn-setting btn-round">Add New Banner</a>
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  <thead>
							  <tr>
                                   <th width="20">&nbsp;</th>
                                   <th width="24">&nbsp;</th>
                                    <th width="214">Banner  Image</th>
                                    <th width="214">Banner Added Date</th>
                                    <th width="214">Link</th>
								   <th width="59">Status </th>
							  </tr>
						  </thead>   
						  <tbody>
                          <? $sel_banner_query= mysql_query("select * from tbl_banner");
						  ...
						  ?>
                            ...
                            <td> <a class="btn btn-info" href="update_banner.php?banner_id=<?=$fetch_banner['id']; ?>">
							<i class="icon-pencil" title="Edit"></i></a></td>
                          
								<td class="center"><img src="product_images/<?=$fetch_banner['banner_image'];?>" width="100" height="100" /></td>
                                 <td class="center"><?=$fetch_banner['date_added'];?></td>
                                    <td class="center"><?=$fetch_banner['url_link'];?></td>
									<td class="center">
